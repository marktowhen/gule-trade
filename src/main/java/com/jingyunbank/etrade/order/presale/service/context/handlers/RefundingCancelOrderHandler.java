...
public class RefundingCancelOrderHandler implements IOrderStatusHandler {
	...
	public Result<String> handle(List<Orders> orders) throws Exception {
		...
		orderGoodsService.refreshGoodStatus(ogids, OrderStatusDesc.resolve(orders.get(0).getStatusCode()));
		return Result.ok();
	}
